...
const [drivers] = await conn.execute("SELECT id FROM users WHERE role = 'driver' LIMIT 1");
...
const loads = [
  // 1. AVAILABLE — waiting to be assigned
  {
  ...
  // 4. IN_TRANSIT — second active load
  {
  ...
  // 5. DELIVERED — completed, awaiting invoice
  {
  ...
  // 6. DELIVERED — second completed load
  {
  ...
  // 7. INVOICED — invoice sent, awaiting payment
  {
    clientName: "Coyote Logistics",
  ...
  // 8. PAID — fully completed and paid
  {
    clientName: "Echo Global Logistics",
  ...
  // 9. PAID — second completed payment
  {
    clientName: "Total Quality Logistics (TQL)",
...
console.log(`Inserting ${loads.length} test loads...`);
...
for (const load of loads) {
  await conn.execute(
    ...
    [
      ...
      load.pickupDate, load.deliveryDate, load.createdBy,
    ]
  );
  console.log(`  ✓ [${load.status.toUpperCase()}] ${load.clientName} — $${load.price}`);
}

await conn.end();
console.log("\n✅ Seed complete. 9 loads inserted.");
