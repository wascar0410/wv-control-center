...
export const plaidRouter = router({
  ...
  debugCheckClassification: protectedProcedure
    .input(z.object({ bankAccountId: z.number() }))
    .query(async ({ ctx, input }) => {
      try {
        ...
        const { bankAccountClassifications, bankAccounts } = await import("../../drizzle/schema");
        ...
        const existing = await db
          ...
          .where(eq(bankAccountClassifications.bankAccountId, input.bankAccountId))
          .limit(1);

        if (existing.length > 0) {
          console.log(`[DEBUG] Classification found for account ${input.bankAccountId}:`, existing[0]);
          return {
            found: true,
        ...
        // Classification doesn't exist, create it as 'operating'
        console.log(`[DEBUG] No classification found for account ${input.bankAccountId}, creating as 'operating'`);
        ...
        const result = await db.insert(bankAccountClassifications).values({
          bankAccountId: input.bankAccountId,
          classification: "operating",
          label: `Operating Account - ${account[0].bankName || "Bank"}`,
          description: "Auto-classified as operating account",
          isActive: true,
        });

        console.log(`[DEBUG] Classification created for account ${input.bankAccountId}`);
        return {
          found: false,
          created: true,
          classification: {
        ...
        console.error("[DEBUG] debugCheckClassification error:", err);
        throw err;
      }
    }),
});
