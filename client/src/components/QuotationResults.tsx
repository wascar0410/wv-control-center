        ...
        <CardContent className="pt-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h2 className="text-3xl font-bold">${fixed(totalPrice, 2)}</h2>
            ...
            <table className="w-full text-sm">
              <tbody className="divide-y">
                ...
                <tr className="bg-blue-50 dark:bg-blue-950">
                  ...
                  <td className="px-4 py-3 text-right font-bold text-blue-600 dark:text-blue-400">${fixed(totalPrice, 2)}</td>
                </tr>
                
                {/* Millas */}
                <tr>
                  <td className="px-4 py-3 text-muted-foreground">Millas Totales</td>
                  <td className="px-4 py-3 text-right">{fixed(totalMiles, 1)} mi</td>
                </tr>
                <tr>
                  <td className="px-4 py-3 text-muted-foreground">Millas Cargadas</td>
                  <td className="px-4 py-3 text-right">{fixed(loadedMiles, 1)} mi</td>
                </tr>
                ...
                <tr className="bg-red-50 dark:bg-red-950">
                  ...
                  <td className="px-4 py-3 text-right font-bold text-red-600 dark:text-red-400">
                    -${fixed((totalOperatingCost || estimatedFuelCost + estimatedOperatingCost), 2)}
                  </td>
                </tr>
                <tr>
                  <td className="px-4 py-3 text-muted-foreground pl-8">Combustible ($0.35/mi)</td>
                  <td className="px-4 py-3 text-right">-${fixed(estimatedFuelCost, 2)}</td>
                </tr>
                <tr>
                  <td className="px-4 py-3 text-muted-foreground pl-8">Mantenimiento ($0.65/mi)</td>
                  <td className="px-4 py-3 text-right">-${fixed(estimatedOperatingCost, 2)}</td>
                </tr>
                ...
                {tollDataSource === "google" && (
                  <tr>
                    ...
                    <td className="px-4 py-3 text-right">
                      {estimatedTollCost > 0 ? `-$${fixed(estimatedTollCost, 2)}` : "$0.00 (sin peajes)"}
                    </td>
                  </tr>
                )}
                ...
                <tr className="bg-muted">
                  ...
                  <td className="px-4 py-3 text-right font-bold">${fixed(ratePerLoadedMile?.toFixed(2) || (totalPrice / loadedMiles), 2)}/mi</td>
                </tr>
                ...
                <tr>
                  ...
                  <td className="px-4 py-3 text-right">${fixed(minimumIncome?.toFixed(2) || (loadedMiles * 2.50), 2)}</td>
                </tr>
                ...
                <tr className={differenceVsMinimum && differenceVsMinimum < 0 ? "bg-yellow-50 dark:bg-yellow-950" : ""}>
                  ...
                  <td className={`px-4 py-3 text-right font-bold ${differenceVsMinimum && differenceVsMinimum >= 0 ? "text-green-600 dark:text-green-400" : "text-yellow-600 dark:text-yellow-400"}`}>
                    {differenceVsMinimum !== undefined ? (differenceVsMinimum >= 0 ? "+" : "") + fixed(differenceVsMinimum, 2) : "N/A"}
                  </td>
                </tr>
              </tbody>
            </table>
            ...
            <ResponsiveContainer width="100%" height={250}>
              <PieChart>
                <Pie
                  ...
                  cy="50%"
                  labelLine={false}
                  label={({ name, value }) => `${name}: $${fixed(value, 2)}`}
                  outerRadius={80}
                  fill="#8884d8"
                  dataKey="value"
                >
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="p-4 rounded-lg bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800">
              ...
              <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">{fixed(emptyMiles, 1)}</p>
            ...
            <div className="p-4 rounded-lg bg-green-50 dark:bg-green-950 border border-green-200 dark:border-green-800">
              ...
              <p className="text-2xl font-bold text-green-600 dark:text-green-400">{fixed(loadedMiles, 1)}</p>
            ...
            <div className="p-4 rounded-lg bg-orange-50 dark:bg-orange-950 border border-orange-200 dark:border-orange-800">
              ...
              <p className="text-2xl font-bold text-orange-600 dark:text-orange-400">{fixed(returnEmptyMiles, 1)}</p>
            ...
            <div className="p-4 rounded-lg bg-purple-50 dark:bg-purple-950 border border-purple-200 dark:border-purple-800">
              ...
              <p className="text-2xl font-bold text-purple-600 dark:text-purple-400">{fixed(totalMiles, 1)}</p>
        ...
        <CardContent className="space-y-2">
          {verdict === "ACEPTAR" && (
            <>
              ...
              <p className="text-sm text-muted-foreground">Con un margen del {fixed(profitMarginPercent, 1)}%, esta carga es altamente rentable y se recomienda aceptarla.</p>
            </>
          )}
          {verdict === "NEGOCIAR" && (
            <>
              <p className="text-yellow-700 dark:text-yellow-300">⚠ Esta carga tiene un margen moderado.</p>
              <p className="text-sm text-muted-foreground">Con un margen del {fixed(profitMarginPercent, 1)}%, considera negociar una tarifa más alta. La diferencia vs mínimo es ${fixed(differenceVsMinimum || 0, 2)}.</p>
            </>
          )}
          {verdict === "RECHAZAR" && (
            <>
              <p className="text-red-700 dark:text-red-300">✗ Esta carga tiene un margen bajo.</p>
              <p className="text-sm text-muted-foreground">Con un margen del {fixed(profitMarginPercent, 1)}%, no es rentable. Rechaza o negocia una tarifa significativamente más alta.</p>
            </>
          )}
        </CardContent>
      </Card>
